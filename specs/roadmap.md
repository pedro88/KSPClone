...
- **M0 — Skeleton (the spine):** all sim + persistence logic landed and unit-tested, and the spine is now **assembled and integration-tested in-process**: `SimCore.ServerSimulation` composes the warp-vote FSM, auto-limit, POI scan, SOI re-parenting, snapshot emitter and connection registry and drives them per fixed tick; `ServerBootstrap` restores-or-seeds the world (`WorldSeed`) and runs the loop with optional Postgres write-through. `ServerSimulationTests` exercises connect → handshake → solo warp → auto-limit-at-SOI → re-parent end-to-end. Persistence is now **verified against Postgres** (the 7 EditMode persistence tests pass with `docker compose up -d`), so PERSIST-1/2/3 are live. The **transport + client also landed**: `KSPClone.Net` defines the P-1 wire codec (full snapshots, lossless round-trip), a transport abstraction with a loopback double (integration-tested: connect → handshake → snapshot stream → warp command) and a **real LiteNetLib UDP** implementation; `ServerBootstrap` hosts it (`ServerNetHost`) and a `KSPClone.Client` (`ClientBootstrap` + overlay) connects over UDP, interpolates snapshots, and sends warp commands. The full server→wire→client demo runs over real UDP (single-process localhost today). Remaining for M0: **T1** (headless `-batchmode` build + `ServerBuilder`, needs Editor) for a true separate-process / multi-machine run. Known deferral: no on-rails orbit/SOI-change replication channel yet, so a client's vessel `parent`/elements come only from the connect handshake (position is streamed); add the event channel with M1. Warp policy: **ADR-0010 amended** — bands are contiguous (≤4× physics, >4× on-rails), no rejected gap.
