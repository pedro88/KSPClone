...
- **M3 — Collaborative VAB:** in progress on `feat/m3-collaborative-vab`. **M3-T01 done** — the `KSPClone.Construction` assembly (part-tree Design model: `Design`/`PartNode`/`PartTree`/`PartCatalog`, engine-agnostic) with an asmdef that references *nothing* — the machine-checkable Art. 7 boundary (BUILD-1). Next: T02 edit-op mutator.
