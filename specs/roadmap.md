...
### VAB wiring + stock parts + launch (post-M3, on `feat/m3-collaborative-vab`)
- [x] Transport dispatch — `ServerVabHost` routes the Design channel (tags 5–15) into the coordinator + locks + launch and sends acks/broadcasts/snapshots; `ClientNetPeer` sends edits/lock/launch and surfaces incoming messages; `ClientVabModel` drives a `DesignReplica`.
- [x] Stock parts — `StockParts` KSP-like catalogue (pod, FL-T400/T800 tanks, LV-T45/LV-909 engines, decoupler, nose cone) with mass/thrust; `LaunchInstantiator` builds engines + wet mass so the craft flies.
- [x] VAB UI — press **B** in the client: browse parts, add/remove/move under a selected node, claim/release a subtree lock, and **LAUNCH**. The launcher takes control of the new vessel and it settles on the M2.5 pad (any vessel promoted near the surface gets a ground pad).

> **How to test the VAB now:** run the client, press **B** to open the VAB, add parts to the demo rocket (two clients editing the same Design converge; one can lock the upper stage), then **LAUNCH** — you take control of the new craft on the pad and fly it (Shift throttle, WASD/QE). Still **runtime-pending**: `DesignStore.UpsertDesign` write-through isn't yet called per accepted op (persistence is built, not wired), and the whole path is validated in-editor next (no Unity in the authoring env). Logic is unit-tested in `Assets/Tests/EditMode/Construction/`.
